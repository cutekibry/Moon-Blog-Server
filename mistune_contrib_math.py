...
# Remodified by Tsukimaru.
# I completely don't know why there is a block math rule,
# Because it doesn't affect anything at all,
# And it will crush if you're trying to add it to the
# Front of the rules.
# So I just simply commented them.
...
class MathBlockMixin(object):
    ...
    def enable_math(self):
        self.rules.block_math = re.compile(r'^\$\$([\s\S]*?)\$\$', re.DOTALL)
...
class MathInlineMixin(object):
    ...
    def enable_math(self):
        self.rules.math = re.compile(r'^\$([\s\S]*?[^\\])\$')
